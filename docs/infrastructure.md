...
- `backend/pyproject.toml` and `backend/uv.lock` exist. The importable ASGI application is `app.main:app`. Alembic uses `backend/alembic.ini`. `DATABASE_URL` is the frozen async URL; backend configuration derives the same host/database with the `postgresql+psycopg` driver for Alembic and synchronous work. The Celery instance is `app.jobs.celery_app:celery_app`; the locked dependency is shared by worker and Beat.
...
If an application workstream changes module or script names, update Compose and this contract in one focused integration commit.
