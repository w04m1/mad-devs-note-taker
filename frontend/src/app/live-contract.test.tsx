...
function responseFor(url: string, malformedTags = false) {
  if (url.includes("/settings"))
    return { email: "demo@example.test", timezone: "UTC", version: 1 };
  ...
  vi.stubGlobal(
    ...
    vi.fn(
      async (input: RequestInfo | URL) =>
        new Response(
          JSON.stringify(responseFor(String(input), malformedTags)),
...
describe("live HTTP shape integration", () => {
  ...
  it("keeps the shell available if tags use a malformed empty response", async () => {
